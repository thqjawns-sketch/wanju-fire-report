...
(function(){
  ...
  function injectStyle(){
    ...
    const st=document.createElement('style');st.id='wanjuTimeWheelStyle';st.textContent=`
      ...
      .actionLine{grid-template-columns:138px minmax(0,1fr) 52px!important}.actionTime{width:100%!important;min-width:0!important;white-space:nowrap;text-align:center!important;padding-left:8px!important;padding-right:8px!important}
      ...
      @media(max-width:650px){.actionLine{grid-template-columns:126px minmax(0,1fr) 48px!important}}
  ...
  function redrawRowsMinute(){
    ...
    if(actions){actions.forEach((a,i)=>{a.time=minuteOnly(a.time||(a.type==='dispatch'?document.getElementById('recv')?.value:''));const e=document.getElementById('actionTime_'+i);if(e){try{e.type='text'}catch(_){}e.value=a.time;e.placeholder='00:00';e.readOnly=true;e.inputMode='none';e.autocomplete='off';e.classList.add('timeWheelInput')}})}
    syncActions();
  }
...
(function(){
  function loadPolish(){
    ...
    const p=document.createElement('script');p.id='wanjuPolishScript';p.src='app-v3-polish.js?v=20260908c';p.defer=true;document.head.appendChild(p);
  }
  function loadCause(){
    if(document.getElementById('wanjuFireCauseScript')){setTimeout(loadPolish,180);return}
    const c=document.createElement('script');c.id='wanjuFireCauseScript';c.src='app-v3-fire-cause.js?v=20260908c';c.defer=true;c.onload=()=>setTimeout(loadPolish,160);document.head.appendChild(c);setTimeout(loadPolish,750);
  }
  function loadOverview(){
    if(document.getElementById('wanjuOverviewBuilderScript')){setTimeout(loadCause,80);return}
    const o=document.createElement('script');o.id='wanjuOverviewBuilderScript';o.src='app-v3-overview-builder.js?v=20260908c';o.defer=true;o.onload=loadCause;document.head.appendChild(o);setTimeout(loadCause,550);
  }
  ...
  const s=document.createElement('script');s.id='wanjuRelationsScript';s.src='app-v3-relations.js?v=20260908c';s.defer=true;s.onload=loadOverview;document.head.appendChild(s);
})();